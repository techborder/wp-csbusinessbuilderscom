...
var wpcfAccess = window.wpcfAccess;
...
$.typesSuggest = function($input, $container, $dropdown, $img, ajax_action, callback_select) {
    var action = ajax_action,
        ...
        selected_values = {},
        prevLength=$input.val().length, xhr=null;
        ...
        xhr = $.ajax({
            ...
            success: function(response) {
                ...
                if (count > 0) {
                    ...
                    $dropdown.append(options).attr('size', resize)
...
$(document).ready(function(){
    ...
    $('.types-suggest').find('.confirm').click(function(){
    ...
    $('.types-suggest').find('.cancel').click(function(){
        var container = $(this).parents('.types-suggest');
        container.find('.dropdown').css('visibility', 'hidden');
        container.find('.toggle').hide();
    });
...
function typesSuggestMarkAsSuggested(selector, parent) {
    $(selector).each(function(){
        var $el=$(this), id = $el.closest(parent).find('.types-suggest').attr('id');
